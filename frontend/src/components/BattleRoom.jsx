...
export default function BattleRoom({ socket, battleId }) {
  ...
  const filtered = filter === 'all'? allGifts : allGifts.filter(g => {
    if (filter === 'legendary') return g.legendary
    return g.category === filter
  })

  useEffect(() => {
    if (!socket) return
    const onLegendary = (data) => {
      setCurrentGift(data)
      setTimeout(() => setCurrentGift(null), 2300)
    }
    socket.on('gift:legendary', onLegendary)
    socket.on('gift', (data) => {
      if (data.legendary) onLegendary(data)
    })
    return () => {
      socket.off('gift:legendary', onLegendary)
      socket.off('gift')
    }
  }, [socket])

  const sendGift = (gift) => {
    if (socket) socket.emit('gift:send', { battleId, giftId: gift.id, legendary: gift.legendary })
    if (gift.legendary || gift.club) {
      setCurrentGift({ type: gift.id, sender: 'You', club: gift.club })
      setTimeout(() => setCurrentGift(null), 2300)
    }
  }
    ...
    <div className="room" style={{ backgroundImage: `url(${bg.file})` }}>
      ...
      <div className="topbar">
        ...
        <div className="bglist">
          {backgrounds.map(b => (
            <button key={b.id} onClick={()=>setBg(b)} className={bg.id===b.id?'active-bg':''}>
      ...
      <style jsx>{`
       ...
       .gift-bar small{display:block;color:#aaa;font-size:10px;text-align:center}
      `}</style>
    </div>
  )
}
