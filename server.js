§§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('dotenv').config();
const express = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('express');
const cors = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('cors');
const helmet = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('helmet');
const morgan = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('morgan');
const rateLimit = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('express-rate-limit');
const path = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('path');
const { v4: uuidv4 } = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('uuid');
...
const friends = [];
const groups = [];
const gifts = §§secret(POSTGRESQL://NEONDB_OWNER:NPG_NX6JVZR2QMYG@EP-FALLING-PAPER-A62FLHPD.US-WEST-2.AWS.NEON.TECH/NEONDB?SSLMODE)('./gifts-500').gifts || [];
...
app.get('/api/trending/hashtags', (req, res) => {
  res.json({
    trending: ['#fyp', '#viral', '#nvme', '#creator', '#dance', '#comedy', '#lifestyle', '#fitness']
  });
});
...
app.get('/api/posts', (req, res) => {
  res.json({ posts: posts.slice(0, 50), count: posts.length });
});
...
app.post('/api/chats', (req, res) => {
  const chat = {
    ...
    type: req.body.type || 'direct', // direct, group
    name: req.body.name,
    createdAt: new Date().toISOString()
  };
  chats.push(chat);
...
app.post('/api/snaps', (req, res) => {
  const snap = {
    ...
    viewDuration: req.body.viewDuration || 10,
    expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
  };
  snaps.push(snap);
...
app.get('/api/tweets', (req, res) => {
  res.json({ tweets: tweets.slice(0, 50) });
});
...
app.get('/api/friends/:userId', (req, res) => {
  const userFriends = friends.filter(f =>
    (f.fromId === req.params.userId || f.toId === req.params.userId) && f.status === 'accepted'
  );
...
// ─── DATING FEATURES (menus.ai-style) ─────────────────────────────────────────
...
app.get('/api/dating/discover/:userId', (req, res) => {
  // Return potential matches based on preferences
...
app.post('/api/dating/swipe', (req, res) => {
  const swipe = {
    ...
    direction: req.body.direction, // 'left' = pass, 'right' = like
    createdAt: new Date().toISOString()
  };

  if (swipe.direction === 'right') {
    // Check for mutual match
    const mutual = matches.find(m =>
      m.swiperId === swipe.swipedId && m.swipedId === swipe.swiperId && m.direction === 'right'
    );
    if (mutual) {
      return res.json({ success: true, match: true, message: 'Its a match!' });
    }
  }
...
app.get('/api/dating/matches/:userId', (req, res) => {
  const userMatches = matches.filter(m =>
    (m.swiperId === req.params.userId || m.swipedId === req.params.userId) && m.direction === 'right'
  );
...
app.post('/api/dating/video-date', (req, res) => {
  res.json({
    ...
    session: {
      ...
      participants: req.body.participants,
      startedAt: new Date().toISOString(),
      duration: 300 // 5 min speed date
    }
  });
});

// ─── CREATOR SIGNUP & PROFILES ────────────────────────────────────────────────
...
app.get('/api/creators/:id', (req, res) => {
  res.json({
    id: req.params.id,
    username: 'creator_' + req.params.id.slice(0, 8),
...
app.post('/api/gifts/send', (req, res) => {
  res.json({
    ...
    transaction: {
      ...
      creatorShare: req.body.amount * 0.55,
      platformShare: req.body.amount * 0.45,
      createdAt: new Date().toISOString()
    }
  });
});
...
// ─── HUGE AI AVATAR DISPLAY (Big animated gifts) ──────────────────────────────
